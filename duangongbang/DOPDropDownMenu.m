...
@property (nonatomic, assign) NSArray *titlesOfMenu;
@property (nonatomic, assign) CGPoint origin;
@property (nonatomic, strong) UIView *backGroundView;
@property (nonatomic, strong) UIView *selectedButtomView;
...
#define kButtomImageViewHeight 44
#define kBackBlueColor 0x02a8f3
#define kLineBlueColor 0x35b9f8
#define kTextColor [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1]
#define kCellTextColor [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1]
...
#define kTextSelectColor [UIColor colorWithRed:56.0f / 255.f green:134.0f / 255.f blue:190.0f / 255.f    alpha:1.0f]
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    if ([_dataSource respondsToSelector:@selector(titlesOfMenu:)]) {
        _titlesOfMenu = [_dataSource titlesOfMenu:self];
    } else {
        _titlesOfMenu = nil;
    }
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:UIColorFromRGB(xMainBlackColor) andPosition:bgLayerPosition];
        [self.layer addSublayer:bgLayer];
        [tempBgLayers addObject:bgLayer];
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval , self.frame.size.height / 1.5);
...
//        }
        titleString = [_titlesOfMenu objectAtIndex:i];
        ...
        //separator
        if (i != _numOfMenu - 1) {
            CGPoint separatorPosition = CGPointMake(ceilf((i + 1) * separatorLineInterval-1), self.frame.size.height / 1.5);
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _show = NO;
        _fontSize = (mainScreenWidth / 320) * 10;//字体大小
//        _separatorColor = UIColorFromRGB(kLineBlueColor);
        _separatorColor = [UIColor whiteColor];
        _textColor = kTextColor;
        _textSelectedColor = kCellTextColor;
        ...
        _leftTableView.rowHeight = (mainScreenWidth / 320) *kTableViewCellHeight;
        ...
        _rightTableView.rowHeight = (mainScreenWidth / 320) *kTableViewCellHeight;
        ...
        _buttomImageView.backgroundColor = UIColorFromRGB(0xf4f4f4);
        _buttomImageView.image = [UIImage imageNamed:@"Category_Buttom_Img"];
        _buttomImageView.contentMode = UIViewContentModeCenter;
        
        //self tapped
        self.backgroundColor = UIColorFromRGB(kBackBlueColor);
        ...
        _selectedButtomView = [[UIView alloc] initWithFrame:CGRectZero];
...

{
    ...
    CGRect rect=CGRectMake(0.0f, 0.0f, 640.0f, 10.0f);
...
- (CGSize)calculateTitleSizeWithString:(NSString *)string
{
    ...
    NSDictionary *dic = @{NSFontAttributeName: [UIFont fontWithName:kFontName size:_fontSize]};
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    if (tapIndex == _currentSelectedMenudIndex && _show) {
        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
            ...
            _show = NO;
            _selectedButtomView.hidden = YES;
            [_selectedButtomView removeFromSuperview];
        }];
    } else {
        _selectedButtomView.frame = CGRectMake((tapIndex)*mainScreenWidth/_numOfMenu, 58.0, mainScreenWidth/_numOfMenu, 6);
        _selectedButtomView.hidden = NO;
        _selectedButtomView.backgroundColor = [UIColor whiteColor];
        _currentSelectedMenudIndex = tapIndex;
...
//        
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                 _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2, mainScreenHeight - 179);
                
                _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2, mainScreenHeight - 179);
            } else {
                _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, mainScreenHeight - 179);
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        cell.textLabel.highlightedTextColor = kCellTextColor;
    ...
    cell.textLabel.font = [UIFont fontWithName:kFontName size:_fontSize];
