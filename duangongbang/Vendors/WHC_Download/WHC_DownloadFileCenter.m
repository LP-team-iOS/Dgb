...
#define kWHC_FilePathCreateFailTxt  (@"DownloadFileCenter ：文件存储路径创建失败")
#define kWHC_FilePathErrorTxt       (@"DownloadFileCenter ：文件存储路径错误不能为空")
...
- (BOOL)createFileSavePath:(NSString *)savePath{
    ...
    if(savePath != nil && savePath.length > 0){
        ...
        if(![fm fileExistsAtPath:savePath]){
            ...
            if(error){
                result = NO;
                LOG(kWHC_FilePathCreateFailTxt);
            }
        }
    }else{
        result = NO;
        LOG(kWHC_FilePathErrorTxt);
    }
    return result;
}
@end
