...
- (void)followScrollView:(UIView *)scrollableView usingTopConstraint:(NSLayoutConstraint *)constraint withDelay:(float)delay {
    ...
    if (!self.overlay) {
        ...
        if (self.navigationController.navigationBar.barTintColor) {
            ...
            LOG(@"[%s]: %@", __PRETTY_FUNCTION__, @"[AMScrollingNavbarViewController] Warning: no bar tint color set");
        }

        if ([self.navigationController.navigationBar isTranslucent]) {
            LOG(@"[%s]: %@", __PRETTY_FUNCTION__, @"[AMScrollingNavbarViewController] Warning: the navigation bar should not be translucent");
        }
...
- (void)appearTheViewAndShowNavBar {
    [self.navigationController.navigationBar addSubview:self.overlay];
}
- (void)disappearTheViewAndShowNavBar {
    [self.overlay removeFromSuperview];
}
